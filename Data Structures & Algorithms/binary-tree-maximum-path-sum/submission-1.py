...
class Solution:
    def maxPathSum(self, root: Optional[TreeNode]) -> int:
        ...
        res = [root.val] # we let the initial known max path sum to only include the root. Using a list as a trick to achieve global variable

        dfs(root) # start solving at the root
        return res[0]
