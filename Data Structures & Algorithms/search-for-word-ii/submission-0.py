'''
Most obvious bruteforce solution: check every single grid and path for match.
More optimal solution uses a Trie. How does that work?
...
class Solution:
    def findWords(self, board: List[List[str]], words: List[str]) -> List[str]:
        ...
        # path from all possible starting cell
        for r in range(ROWS):
            for c in range(COLS):
                dfs(r, c, root, "")
